...
export function initPhoneUI() {
    const $container = $('#tamako-phone-standalone');
    ...
    $container.html(`
        ...
        <div class="tamako-phone-resize tamako-phone-resize-e" data-dir="e"></div>
        <div class="tamako-phone-resize tamako-phone-resize-se" data-dir="se"></div>
    `);
...
// ===== 手机外观设置存储（存在玉子市场自己的 extensionSettings 中）=====

export function getPhoneSettings() {
    try {
        const ctx = SillyTavern?.getContext?.();
        const ext = ctx?.extensionSettings?.TamakoMarket;
        return ext?.phone || {};
    } catch (e) {
        return {};
    }
...
export function savePhoneSetting(key, value) {
    try {
        const ctx = SillyTavern.getContext();
        const ext = ctx.extensionSettings.TamakoMarket;
        if (!ext.phone) ext.phone = {};
        ext.phone[key] = value;
        ctx.saveSettingsDebounced?.();
    } catch (e) {
        console.warn('[玉子的手机] 保存设置失败:', e);
    }
}
